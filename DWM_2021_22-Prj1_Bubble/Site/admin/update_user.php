...
if (isset($_GET['delete_userid'])) {
    ...
    $user_delete->bind_param("ii", $estado, $userid);
    $user_delete->execute();
    $user_delete->close();

    array_push($_SESSION['alerts']['alert'], 'Utilizador banido com sucesso!');
    header('location:./users.php');
    exit;
}

if (isset($_GET['remove_ban_userid'])) {

    $userid = intval($_GET['remove_ban_userid']);
    ...
    array_push($_SESSION['alerts']['alert'], 'Banimento removido do utilizador com sucesso sucesso!');
    header('location:./users.php');
    exit;
}
